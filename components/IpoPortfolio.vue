...
const chartColors = ['#4F6B9F', '#7C5EAF', '#2E8B8B', '#C88B3F', '#C34A4A', '#3A9B9B'];
    ...
    options: {
      ...
      plotOptions: {
        treemap: {
          distributed: true,
          colorScale: {
            ranges: seriesData.map((_item, index) => ({
              from: -1,
              to: -1,
...
const currentChart = computed(() => {
  switch (activeChart.value) {
    case 'treemap':
      return treemapChart.value;
    case 'radialBar':
      return radialBarChart.value;
    case 'donut':
    default:
      return donutChart.value;
  }
});
        ...
        <ul v-else class="item-list-scroll-wrapper">
          <li v-for="holding in portfolioStore.holdings" :key="holding.id" class="holding-item">
            <div class="item-info">
              ...
              <span class="item-meta">{{ holding.unitsAllotted }} units @ {{
              ...
              <button @click="promptEdit(holding)" class="action-btn" title="Edit Holding">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                  ...
                  <path stroke-linecap="round" stroke-linejoin="round"
                        d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 19.08a2.25 2.25 0 01-1.06.669l-3.328.788a.75.75 0 01-.976-.976l.788-3.328a2.25 2.25 0 01.669-1.06l12.13-12.132z"/>
                </svg>
              </button>
...
.action-btn:not(.remove-btn):hover {
